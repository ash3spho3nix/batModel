...
% I. Setup the GA
ff='myfunc_bat_1C_charge'; % objective function
...
maxit=50000; % max number of iterations
...
disp('_________________________________________');
xx=load('v_C_1C_charge.mat');
V_exp=xx.v_C_1CC(:,2);
C_exp=xx.v_C_1CC(:,1);
C=1;
s=one_cycle_kokam_GA_charge(par(1,:),C,C_exp(end),V_exp(1));
figure(25);
plot(s.Cs,s.V,'g');
hold on;
plot(C_exp,V_exp,'r');
