...
% I. Setup the GA
% matf=['v_C_T_0.mat';'v_C_T_25.mat';'v_C_T_40.mat';'v_C_T_55.mat'];
% l=['myfunc_bat_C0T';'myfunc_bat_C25T';'myfunc_bat_C40T';'myfunc_bat_C55T'];
Tmp=[0;25;40;55];
% for temperature=1:4
ff='myfunc_bat_C40T'; % objective function
npar=16; % number of optimization variables
hi=10*ones(1,npar);
lo=-0.1*ones(1,npar);
%_______________________________________________________
...
% csvwrite([l(temperature) '.dat'],par(1,:));
format short g
...
disp('_________________________________________');
xx=load('v_C_T_40.mat');
V_exp=xx.v_C_T_40(:,2);
C_exp=xx.v_C_T_40(:,1);
C=0.5;
Temp=40;
a=csvread('c05_fin.dat');
s=one_cycle_kokam_GA_T(a,par(1,:),C,C_exp(end),V_exp(1),Temp);
figure(12);
plot(s.Cs,s.V,'g');
hold on;
plot(C_exp,V_exp,'r');
