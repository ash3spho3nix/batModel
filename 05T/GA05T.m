...
% I. Setup the GA
ff='myfunc_bat_C05T'; % objective function
npar=34; % number of optimization variables
hi_r=10*ones(1,9);
hi_V=10*ones(1,9);
hi_c=10*ones(1,6);
%hi_t=1*ones(1,4);
hi_t=2*ones(1,4);%changed on 21/11
hi_c1=10*ones(1,6);
...
C=0.5;
Temp=40;
s=one_cycle_kokam_GA_new_T(par(1,:),C,C_exp(end),V_exp(1),Temp);
figure(35);
plot(s.Cs,s.V,'g');
hold on;
plot(C_exp,V_exp,'r');
